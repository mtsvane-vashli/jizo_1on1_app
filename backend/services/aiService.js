...
- response_textは意味内容を変えずに読みやすくするため、Markdown形式（例: 重要箇所は**太字**、箇条書きは'-'）を適切に使ってください。過剰な装飾は避け、1段落目・2段落目の区別は空行で保ってください。
...
- response_textは上司に向けたメモとして以下の2段落構成にしてください。
  1. 「**部下の状態推察**:」の行を置き、その直後に改行して内容を記述してください。
  2. 「**上司への提案**:」の行を置き、その直後に改行して内容を記述してください。
- suggested_questionsには、会話の流れを汲み取り、部下の内省を促すオープンクエスチョンを4つ。
- response_textは意味内容を変えずに読みやすくするため、Markdown形式を活用してください。重要語は**太字**、箇条書きや番号付きリスト（必要に応じて'-'や'1.'）で視認性を高め、段落間は空行で区切ってください。
