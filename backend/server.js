...
// 会話の状態を管理する変数
let conversationState = 'initial'; // 'initial', 'theme_selection', 'engagement_selection', 'on_demand' 
let selectedTheme = ''; // ユーザーが選択したテーマを保存 
let selectedEngagement = ''; // ユーザーが選択した関わり方を保存 
...
app.post('/api/chat', async (req, res) => {
    ...
    // ユーザーメッセージをDBに保存 (固定フローの内部メッセージを除く) 
    // "__START__" はフロントエンドからの開始シグナルであり、ユーザーの直接の入力ではないため保存しない 
    if (userMessage !== '__START__' && currentConversationId) { // currentConversationId が設定されていることを確認
        db.run('INSERT INTO messages (conversation_id, sender, text) VALUES (?, ?, ?)',
            [currentConversationId, 'user', userMessage], function(err) {
    ...
    if (userMessage === '__START__') {
        ...
        db.run('INSERT INTO conversations (theme, engagement) VALUES (?, ?)',
            ['初期設定中', '初期設定中'], function(err) { // 初期テーマと関わり方 
            if (err) {
                console.error('Error starting new conversation:', err.message);
                return res.status(500).json({ error: 'Failed to start conversation.' });
            }
            currentConversationId = this.lastID; // 新しく挿入された行のIDを取得 
            // AIの最初の返信をDBに保存 
            db.run('INSERT INTO messages (conversation_id, sender, text) VALUES (?, ?, ?)',
                [currentConversationId, 'ai', aiReply], function(err) {
                if (err) console.error('Error saving AI reply:', err.message);
            });
            return res.json({ reply: aiReply });
        });
        return; // ここで処理を終了し、Gemini API呼び出しをスキップ 
    } else if (conversationState === 'theme_selection') {
        selectedTheme = userMessage; // ユーザーが選択したテーマを保存 
        conversationState = 'engagement_selection';
        ...
        // 会話のテーマを更新 
        db.run('UPDATE conversations SET theme = ? WHERE id = ?',
            [selectedTheme, currentConversationId], function(err) {
            if (err) console.error('Error updating theme:', err.message);
        });
        // AIの返信をDBに保存 
        db.run('INSERT INTO messages (conversation_id, sender, text) VALUES (?, ?, ?)',
            [currentConversationId, 'ai', aiReply], function(err) {
        ...
        return res.json({ reply: aiReply });
    } else if (conversationState === 'engagement_selection') {
        selectedEngagement = userMessage; // ユーザーが選択した関わり方を保存 
        conversationState = 'on_demand'; // 通常のオンデマンドモードへ移行 
        aiReply = `承知いたしました。選択いただいた【テーマ:${selectedTheme}】と【関わり方:${selectedEngagement}】を念頭に置き、サポートさせていただきます。本日はこのテーマと関わり方で進めてまいります。
        ...
        db.run('UPDATE conversations SET engagement = ? WHERE id = ?',
            [selectedEngagement, currentConversationId], function(err) {
    ...
    // 通常のオンデマンドモードのプロンプト処理 
    // ここからGemini APIを呼び出すためのプロンプトを構築します
    fullPrompt = `
        ...
        *平易な言葉:専門用語を乱用せず、分かりやすく説明する 
        ...
        現在のテーマ: ${selectedTheme || '未設定'}
        現在の関わり方: ${selectedEngagement || '未設定'}
